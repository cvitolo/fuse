...
```{r}
streamflow <- fuse(DATA, myMID, myDELTIM, parameters[1,])
...
for (i in 1:dim(parameters)[1]){
  ...
  lines(zoo::zoo(fuse(DATA = DATA, 
                      mid = myMID, 
                      deltim = myDELTIM, 
                      ParameterSet = parameters[i,]), 
                 order.by = zoo::index(DATA)),
        col = "gray", lwd = 0.1)
...
discharges <- matrix(NA, ncol = 4*dim(parameters)[1], nrow = dim(DATA)[1])
kCounter <- 0
...
plot(zoo::coredata(DATA$Q), type = "l", lwd=0.5, 
...
plot(zoo::coredata(DATA$Q), type = "l", lwd=1,
     xlab = "", ylab = "Streamflow [mm/day]")
